...
public class SeatRepository : ISeatRepository
{
    ...
    public async Task<IEnumerable<Seat>> GetByCoachIdAsync(int coachId)
        => await _db.Seats.AsNoTracking().Where(s => s.CoachId == coachId).ToListAsync();

    public async Task<Seat?> GetByIdAsync(int id)
        => await _db.Seats.AsNoTracking().FirstOrDefaultAsync(s => s.Id == id);
    ...
    {
        var existing = await _db.Seats.FirstOrDefaultAsync(s => s.Id == seat.Id);
