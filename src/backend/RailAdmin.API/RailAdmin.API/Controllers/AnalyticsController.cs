...
public class AnalyticsController : ControllerBase
{
        ...
        [FromQuery] string range = "Month")
    {
        ...
        // Chỉ tính các vé đã thanh toán thành công
        ...
        // Lấy dữ liệu qua Reservation.JourneyDate
        ...
        result.TotalRevenue = filteredTickets.Sum(t => t.Amount);
        ...
        if (range.Equals("Week", StringComparison.OrdinalIgnoreCase))
        {
            result.Revenue = Enumerable.Range(0, 7).Select(i =>
            {
                ...
                return new RevenueDataDto
                {
                    ...
                    Revenue = filteredTickets.Where(t => t.JourneyDate.Date == date).Sum(t => t.Amount)
                };
            }).ToList();
        ...
        else if (range.Equals("Month", StringComparison.OrdinalIgnoreCase))
        {
            result.Revenue = Enumerable.Range(1, 12).Select(month => new RevenueDataDto
            {
                ...
                Revenue = filteredTickets.Where(t => t.JourneyDate.Month == month).Sum(t => t.Amount)
            }).ToList();
        ...
        else
        {
            ...
            {
                Name = year.ToString(),
                Revenue = filteredTickets.Where(t => t.JourneyDate.Year == year).Sum(t => t.Amount)
            }).ToList();
